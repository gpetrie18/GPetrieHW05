...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate{
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
        ...
        Formula(conversionString: "fahrenheit to censius", formula: {($0 - 32) * (5/9)}),
    ...
    func calculateConversion() {
        ...
        if let inputValue = Double(userInput.text!) {
            ...
            showAlert(title: "Error", message: "Please only enter numeric values, no commas, spaces, symbols, or non-numeric characters")
    ...
    @IBAction func posNegPressed(_ sender: UISegmentedControl) {
        ...
        if posNeg.selectedSegmentIndex == 0 {
